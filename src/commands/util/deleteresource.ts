...
export default class UtilDeleteResource extends Command {
  ...
  async run() {
    ...
    if (flags.ids) {
      resource_ids = utils.splitDedupAndFlatten(flags.ids)
    }
    if (flags.pipe) {
    ...
    for (const failure of r.getFailedIndices()) {
      ...
      console.error(`${chalk.bold.red(`Failed to delete ${flags['resource-type']} `)}${chalk.bold.blue(resource_id)}: ${r.results[failure].getFormattedError()}`)
    }
  }
}
