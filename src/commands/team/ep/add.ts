...
export default class TeamEpAdd extends AuthenticatedBaseCommand<typeof TeamEpAdd> {
  static description = 'Add PagerDuty escalation policies to Teams.'
  ...
  async run() {
    ...
    const r = await this.pd.batchedRequestWithSpinner(requests, {
      activityDescription: `Adding ${ep_ids.length} escalation policies to ${team_ids.length} teams`,
    })
